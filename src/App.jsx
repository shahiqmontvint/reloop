...
function WorkSheetPage({ worksheet, setWorksheet, canEdit }) {
  const cols = worksheet?.cols || ["Date","Description","Type","Amount (₨)","Notes"];
  ...
  const updateCol = (i, val) => {
    const next = cols.map((c,j)=>j===i?val:c);
    save(next, rows);
  };
  const addCol = () => save([...cols,"New Column"], rows);
  const removeCol = i => {
    if(cols.length<=1) return;
    save(cols.filter((_,j)=>j!==i), rows.map(r=>r.filter((_,j)=>j!==i)));
  };

  // Row management
  const addRow = () => save(cols, [...rows, {id:Date.now(), cells:cols.map(()=>"")}]);
  const updateCell = (rowId, colIdx, val) =>
    save(cols, rows.map(r=>r.id===rowId?{...r,cells:r.cells.map((c,j)=>j===colIdx?val:c)}:r));
  ...
  // Summary — try to sum any column that looks numeric
  const numericColIdx = cols.reduce((acc,c,i)=>{
    const vals = rows.map(r=>parseFloat((r.cells||[])[i])||0);
    const hasNums = vals.some(v=>v!==0);
    if(hasNums) acc.push(i);
    return acc;
  },[]);
  ...
  const doExport = fmt => {
    ...
    if(fmt==="csv"){
      ...
      const tableRows = rows.map((r,i)=>`<tr>${(r.cells||[]).map((c,j)=>`<td style="text-align:${numericColIdx.includes(j)?"right":"left"}">${c||""}</td>`).join("")}</tr>`).join("");
      const totalsRow = numericColIdx.length>0?`<tr style="background:#1E1530;color:#C8F135;font-weight:600">${cols.map((c,i)=>numericColIdx.includes(i)?`<td style="text-align:right">${rows.reduce((s,r)=>s+(parseFloat((r.cells||[])[i])||0),0).toLocaleString()}</td>`:`<td>${i===0?"Total":""}</td>`).join("")}</tr>`:"";
      ...
      <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .tagline{color:#9B8FBB;font-size:10px;margin-bottom:6px}
        .meta{color:#9B8FBB;font-size:10px} .meta b{color:#C8F135;font-weight:600}
        ...
        td{padding:7px 12px;border-bottom:0.5px solid #eee;vertical-align:middle;color:#1a1a1a}
      ...
      <div class="header">
        <div>
          ...
          <div class="tagline">Work Sheet</div>
          <div class="meta">Exported: <b>${now}</b></div>
        </div>
  ...
  const IS={padding:"6px 8px",background:"transparent",border:"none",outline:"none",fontSize:12,fontFamily:FB,color:T.offWhite,width:"100%",boxSizing:"border-box"};
  const numSums = cols.map((c,i)=>numericColIdx.includes(i)?rows.reduce((s,r)=>s+(parseFloat((r.cells||[])[i])||0),0):null);

  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      {/* Header */}
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{fontFamily:FB,fontSize:20,fontWeight:700,color:T.offWhite,flex:1}}>📒 Work Sheet</div>
        {canEdit&&<button onClick={addRow} style={{padding:"6px 14px",border:`1px solid ${T.border}`,borderRadius:8,background:T.card,color:T.muted,cursor:"pointer",fontSize:12,fontFamily:FB}} onMouseEnter={e=>{e.currentTarget.style.borderColor=T.lime;e.currentTarget.style.color=T.lime;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color=T.muted;}}>+ Add row</button>}
        {canEdit&&<button onClick={addCol} style={{padding:"6px 14px",border:`1px solid ${T.border}`,borderRadius:8,background:T.card,color:T.muted,cursor:"pointer",fontSize:12,fontFamily:FB}} onMouseEnter={e=>{e.currentTarget.style.borderColor=T.lime;e.currentTarget.style.color=T.lime;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color=T.muted;}}>+ Add column</button>}
        <div style={{display:"flex",border:`1px solid ${T.border}`,borderRadius:8,overflow:"hidden"}}>
          {["csv","pdf"].map((f,i)=>(
            <button key={f} onClick={()=>doExport(f)} style={{padding:"6px 12px",background:T.card,border:"none",borderLeft:i>0?`1px solid ${T.border}`:"none",cursor:"pointer",fontSize:12,color:T.muted,fontFamily:FB}} onMouseEnter={e=>{e.currentTarget.style.color=T.lime;e.currentTarget.style.background=T.surface;}} onMouseLeave={e=>{e.currentTarget.style.color=T.muted;e.currentTarget.style.background=T.card;}}>↓ {f.toUpperCase()}</button>
          ))}
        </div>
      </div>

      <div style={{flex:1,overflowX:"auto",overflowY:"auto",background:T.bg}}>
        {rows.length===0&&!canEdit
          ?<div style={{textAlign:"center",padding:"80px 20px"}}><div style={{fontSize:36,marginBottom:12}}>📒</div><div style={{fontSize:18,color:T.ghost}}>No entries yet</div></div>
          :<table style={{borderCollapse:"collapse",minWidth:"100%",fontSize:12,fontFamily:FB}}>
            <thead>
              <tr style={{background:T.surface,position:"sticky",top:0,zIndex:2}}>
                {cols.map((col,i)=>(
                  <th key={i} style={{padding:"10px 12px",borderBottom:`2px solid ${T.lime}40`,borderRight:`1px solid ${T.border}`,textAlign:"left",whiteSpace:"nowrap",minWidth:120,position:"relative"}}>
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      {canEdit
                        ?<input value={col} onChange={e=>updateCol(i,e.target.value)} style={{...IS,fontSize:10,fontWeight:700,color:T.lime,textTransform:"uppercase",letterSpacing:"0.9px",background:"transparent"}}/>
                        :<span style={{fontSize:10,fontWeight:700,color:T.lime,textTransform:"uppercase",letterSpacing:"0.9px"}}>{col}</span>}
                      {canEdit&&cols.length>1&&<button onClick={()=>removeCol(i)} style={{background:"none",border:"none",cursor:"pointer",color:T.ghost,fontSize:11,padding:"0 2px",flexShrink:0,lineHeight:1}} onMouseEnter={e=>e.currentTarget.style.color=T.rougeText} onMouseLeave={e=>e.currentTarget.style.color=T.ghost}>✕</button>}
                    </div>
                  </th>
                ))}
                <th style={{padding:"10px 8px",borderBottom:`2px solid ${T.lime}40`,width:80,minWidth:80}}/>
              </tr>
            </thead>
            <tbody>
              {rows.map((row,ri)=>(
                <tr key={row.id} style={{background:ri%2===0?T.card:T.surface}} onMouseEnter={e=>e.currentTarget.style.background=`${T.lime}08`} onMouseLeave={e=>e.currentTarget.style.background=ri%2===0?T.card:T.surface}>
                  {cols.map((col,ci)=>(
                    <td key={ci} style={{padding:"4px 6px",borderBottom:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}40`,verticalAlign:"middle"}}>
                      {canEdit
                        ?<input value={(row.cells||[])[ci]||""} onChange={e=>updateCell(row.id,ci,e.target.value)} placeholder="—" style={IS}/>
                        :<span style={{fontSize:12,color:T.offWhite,padding:"4px 6px",display:"block"}}>{(row.cells||[])[ci]||"—"}</span>}
                    </td>
                  ))}
                  <td style={{padding:"4px 6px",borderBottom:`1px solid ${T.border}`,width:80,textAlign:"center"}}>
                    <div style={{display:"flex",gap:3,justifyContent:"center",alignItems:"center"}}>
                      <button onClick={()=>moveRow(row.id,-1)} disabled={ri===0} style={{background:"none",border:"none",cursor:ri===0?"default":"pointer",color:ri===0?T.ghost:T.muted,fontSize:12,padding:"2px 4px"}} onMouseEnter={e=>ri>0&&(e.currentTarget.style.color=T.lime)} onMouseLeave={e=>e.currentTarget.style.color=ri===0?T.ghost:T.muted}>↑</button>
                      <button onClick={()=>moveRow(row.id,1)} disabled={ri===rows.length-1} style={{background:"none",border:"none",cursor:ri===rows.length-1?"default":"pointer",color:ri===rows.length-1?T.ghost:T.muted,fontSize:12,padding:"2px 4px"}} onMouseEnter={e=>ri<rows.length-1&&(e.currentTarget.style.color=T.lime)} onMouseLeave={e=>e.currentTarget.style.color=ri===rows.length-1?T.ghost:T.muted}>↓</button>
                      {canEdit&&<button onClick={()=>deleteRow(row.id)} style={{background:"none",border:"none",cursor:"pointer",color:T.ghost,fontSize:13,padding:"2px 4px",lineHeight:1}} onMouseEnter={e=>e.currentTarget.style.color=T.rougeText} onMouseLeave={e=>e.currentTarget.style.color=T.ghost}>✕</button>}
                    </div>
                  </td>
                </tr>
              ))}
              {/* Totals row */}
              {numericColIdx.length>0&&rows.length>0&&(
                <tr style={{background:T.surface,position:"sticky",bottom:0}}>
                  {cols.map((c,i)=>(
                    <td key={i} style={{padding:"8px 12px",borderTop:`2px solid ${T.lime}40`,borderRight:`1px solid ${T.border}`,fontWeight:700,color:numSums[i]!==null?T.lime:T.ghost,fontSize:12,textAlign:numericColIdx.includes(i)?"right":"left"}}>
                      {i===0?"TOTAL":numSums[i]!==null?numSums[i].toLocaleString():""}
                    </td>
                  ))}
                  <td style={{borderTop:`2px solid ${T.lime}40`}}/>
                </tr>
              )}
            </tbody>
          </table>}
  ...
  return(
    ...
    >
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,background:T.bg,height:"100%",overflow:"hidden"}}>
        ...
        :activePage==="worksheet"?(isFinAdmin?<WorkSheetPage worksheet={worksheet} setWorksheet={w=>{setWorksheet(w);sbSet({brands,items,nid,catTree,fixes,rates,rateHistory,bundles,attendance,orders,worksheet:w});}} canEdit={isFinAdmin} isAdmin={isAdmin}/>:<AccessDenied/>)
        :(
          <>
            {/* Top bar */}
            <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"12px 20px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
